...
public class DateEx {
	...
	//2. Calendar
		...
		int 월 = 캘린더.get(Calendar.MONTH); //0월부터 가져오기 떄문에 +1 해줘야함
		...
		System.out.println("날짜 : " + 날짜 ); //5월 23일이 목요일이기때문에 5라고 나오는 것
	
		/*
		 4. SimpleDateFormat  년 월 일
		 년   : y	year
		 월   : M	Month (분이랑 첫 글자가 m인 것이 같기 때문에 월은 대문자 M임)
		 일   : d	day
		 시   : H	Hour  (H : 0~23 시		h : 오전(am)과 오후(pm)로 나눠서 표기)
		 분   : m	minute
		 초   : s	second
		 
		 요일 : E E라는 글자로 표기하자 국제표준화기구에서 약속
		 		EEEE는 오늘 요일
		 */
